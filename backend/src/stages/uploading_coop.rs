use crate::models::{ChangelogInsert, CoopBundledInsert, CoopTempUser, PostCoop, SpPbHistory};
use anyhow::Result;
use log::debug;
...
pub fn post_coop_pb(params: PostCoop) -> Result<()> {
    ...
    if let Some(profile_number2) = profile_number2 {
        ...
        let (score1, temp_usr_data) = construct_coop_score_from_one(params)?;
        let post_url = "http://localhost:8080/api/v1/sp/post_score".to_string();
        let new_id1 = client.post(&post_url).json(&score1).send()?.json::<i64>()?;
        let bundle = CoopBundledInsert {
            p_id1: profile_number1,
            p_id2: Some(temp_usr_data.profile_number),
            p1_is_host: None,
            cl_id1: new_id1,
            cl_id2: Some(temp_usr_data.cl_id),
        };
        let post_url = "http://localhost:8080/api/v1/coop/post_score".to_string();
        let _res = client.post(&post_url).json(&bundle).send()?.json::<i64>()?;
    }
...
pub fn construct_coop_score_from_one(params: PostCoop) -> Result<(ChangelogInsert, CoopTempUser)> {
    ...
    // TODO: We need to grab the filler score here I think.
